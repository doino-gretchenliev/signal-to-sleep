...
Connects via WebSocket to port 1884 with username/password auth.

Usage:
  python generate_test_data.py [host] [port] [topic] [username] [password]
"""
...
BROKER_PORT = int(sys.argv[2]) if len(sys.argv) > 2 else int(os.environ.get("MQTT_PORT", "1884"))
...
# Connect to broker
client = mqtt.Client(client_id="test-data-gen", transport="websockets")
